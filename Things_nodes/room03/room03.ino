...
//  }
  if (str.substring(0,7) == "room:03"){ //tu ki tu so 1 den 7
    str.replace("room:03|","");
    if (str.substring(0,8) == "Id:led01"){
      ...
      if(str.substring(7,9) == "ON"){
        if(digitalRead(D6) == 0){
          ...
          snprintf (msg, 50, "flag:2|room:03|name:03led01|status:ON");
          client.publish(m_topic, msg);
          state = ON;
        }
        ...
        if(digitalRead(D6) == 1){
          digitalWrite(D6, LOW);
          snprintf (msg, 50, "flag:2|room:03|name:03led01|status:OFF");
          client.publish(m_topic, msg);
          state = OFF;
        }
      ...
      if(str.substring(7,9) == "ON"){
        if(digitalRead(D7) == 0){
          ...
          snprintf (msg, 50, "flag:2|room:03|name:03led02|status:ON");
          client.publish(m_topic, msg);
          state2 = ON2;
        }
        ...
        if(digitalRead(D7) == 1){
          digitalWrite(D7, LOW);
          snprintf (msg, 50, "flag:2|room:03|name:03led02|status:OFF");
          client.publish(m_topic, msg);
          state2 = OFF2;
        }       
      }
...
void loop(){
  ...
  if (now - lastMsg > 3000){
    ...
    snprintf (msg, 100, "flag:1|room:03|nhiet:%f|am:%f|lux:%f|smoke:%d", t, h, lux, gassensorAnalog); 
    client.publish(m_topic, msg);
  }
