...
//  }
  if (str.substring(0,7) == "room:01"){ //tu ki tu so 1 den 7
    str.replace("room:01|","");
    if (str.substring(0,8) == "Id:led01"){
      ...
      if(str.substring(7,9) == "ON"){
        if(digitalRead(D6) == 0){
          ...
          snprintf (msg, 50, "flag:2|room:01|name:01led01|status:ON");
          client.publish(m_topic, msg);
          state = ON;
        }
        ...
        if(digitalRead(D6) == 1){
          digitalWrite(D6, LOW);
          snprintf (msg, 50, "flag:2|room:01|name:01led01|status:OFF");
          client.publish(m_topic, msg);
          state = OFF;
        }
      }
      ...
      if(str.substring(7,9) == "ON"){
        if(digitalRead(D7) == 0){
          ...
          snprintf (msg, 50, "flag:2|room:01|name:01led02|status:ON");
          client.publish(m_topic, msg);
          state2 = ON2;
        }       
      }else if(str.substring(7,10) == "OFF"){
        if(digitalRead(D7) == 1){
          digitalWrite(D7, LOW);
          snprintf (msg, 50, "flag:2|room:01|name:01led02|status:OFF");
          client.publish(m_topic, msg);
          state2 = OFF2;
        }      
      }
    }else if (str.substring(0,8) == "Id:led03"){
      str.replace("Id:led03|","");
      if(str.substring(7,10) == "OFF"){
        if(digitalRead(D8) == 1){
          digitalWrite(D8, LOW);
          snprintf (msg, 50, "flag:2|room:01|name:01led03|status:OFF");
          client.publish(m_topic, msg);
        }     
      }
    }
...
void loop(){
  ...
  if (now - lastMsg > 3000){
    ...
    snprintf (msg, 100, "flag:1|room:01|nhiet:%f|am:%f|lux:%f|smoke:%d", t, h, lux, gassensorAnalog); 
    client.publish(m_topic, msg);
  }
